...
from utils import make_image_and_source_list

from fake_image.PiecewiseBicubicPSF import PiecewiseBicubicPSF
...
class TestPiecewiseBicubic(unittest.TestCase) :
    ...
    def check_results(self, psf_fit_fname, sources, extra_variables) :
        ...
        Args:
            - psf_fit_fname:
                The name of the file produced by fitpsf to check the PSF map of.

            - sources:
                The sources argument used to generate the image that was fit.
                See same name argument of run_test.
        ...
        terms = [eval(term, variables)
                 for term in psffit_terms[1:-1].split(',')]
        for i in range(len(terms)) :
            if type(terms[i]) is float or type(terms[i]) is int :
                terms[i] = numpy.full(num_sources, float(terms[i]))
        
        terms = numpy.dstack(terms)[0]
        coefficients = map_group['Coefficients'][:]

        #Indices are: source index, variable, y boundary ind, x boundary ind
        fit_params = numpy.tensordot(terms, coefficients, [1, 3])
        self.assertEqual(
            fit_params.shape, 
...
#        print('Fluxes: ' + repr(fluxes))
    ...
    def run_test(self,
        ...
        Args:
            - sources:
                A list of lists of dictionaries specifying the list of sources
                to fit.
                
                Each list of dictionaries specifies the sources to drop
                on a single image.
                ...
                Each source must contain the following:
                    ...
                    - psf_args:
                        The arguments with which to create the
                        PiecewiseBicubicPSF for the source. See
                        PiecewiseBicubicPSF.__init__ for details.

            - psffit_terms:
                The terms on which PSF parameters depend on. See --psf.terms
                argument of the fitpsf command.

            - extra_variables:
                A list of the variables in addition to x and y that participate
                in the fitting terms.
        ...
        for subpix_map in [numpy.ones((1,1)),
            ...
            source_list_fname = fname_start + 'srclist'
            psf_fit_fname = fname_start + 'hdf5'
            image_fname = fname_start + 'fits'
            subpix_fname = fname_start + 'subpix.fits'

            if exists(source_list_fname) : remove(source_list_fname)
            if exists(subpix_fname) : remove(subpix_fname)
            ...
            for image_index, image_sources in enumerate(sources) :
                make_image_and_source_list(
                    ...
                    image_fname = image_fname + '.' + str(image_index),
                    source_list_fname = source_list_fname,
                    psf_fit_fname = psf_fit_fname + '.' + str(image_index),
                    extra_variables = extra_variables,
                    subpix_map = subpix_map
                )

            hdu_list = pyfits.HDUList([pyfits.PrimaryHDU(subpix_map)])
            hdu_list.writeto(subpix_fname, clobber = True)
            ...
            config_fname = fname_start + 'cfg'
            config = open(config_fname, 'w')
            with open('test_data/config_template.cfg', 'r') as config_template :
                config.write(config_template.read()
                             %
                             fitpsf_config)
            config.close()

            for image_index in range(len(sources)) :
                if exists(psf_fit_fname + '.' + str(image_index)) :
                    remove(psf_fit_fname + '.' + str(image_index))

            self.assertEqual(
                call([self.fitpsf_executable, '-c', config_fname]),
                0
            )

            for image_index, image_sources in enumerate(sources) :
                self.check_results(psf_fit_fname+ '.' + str(image_index),
                                   image_sources,
                                   extra_variables)

            for image_index, image_sources in enumerate(sources) :
                remove(psf_fit_fname+ '.' + str(image_index))
                remove(image_fname+ '.' + str(image_index))

            remove(config_fname)
            remove(source_list_fname)
            remove(subpix_fname)
    ...
    def test_multi_image_with_extra_variables(self) :
