...
class Image(numpy.ndarray) :
    ...
    def add_source(self,
        ...
        Args:
            - x:
                The x coordinate of the center of the new source relative to
            ...
            - amplitude:
                The amplitude to scale the PSF by.

            - psf:
                The PSF to use for the new source. Should be of some type
                inherited from fake_image.psf or None if PRF is used instead.

            - prf:
                The pixel response function (PRF) to use for the new source.
                Should be of some type inherited from fake_image.psf or None
                if PSF is used instead. The pixel response function is the
                PSF convolved with the pixel sensitivity map.

            -subpix_map:
                The sub-pixel sensitivity map. Should be a numpy.ndarray of
                some sort (i.e. provide shape attribute and subscripting).
