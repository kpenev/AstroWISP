...
class TestPiecewiseBicubicPSF(FloatTestCase):
    ...
    def setUp(self):
        ...
        self._grids = [
            dict(x=numpy.array([0.0, 1.0]),
                 y=numpy.array([0.0, 1.0])),
            dict(x=numpy.array([0.0, 0.5]),
                 y=numpy.array([0.0, numpy.pi / 3.0])),
            dict(x=numpy.array([0.0, 0.5, 1.0]),
                 y=numpy.array([0.0, 1.0])),
        ]
        self._test_points = [
            [
    ...
    def test_zero(self):
        ...
        for grid, test_points in zip(self._grids, self._test_points):
            ...
            psf = PiecewiseBicubicPSF(
                psf_parameters=dict(values=numpy.zeros(shape),
    ...
    def test_one(self):
        ...
        for grid, test_points in zip(self._grids, self._test_points):
            shape = (grid['y'].size, grid['x'].size)
            psf = PiecewiseBicubicPSF(
                psf_parameters=dict(values=numpy.ones(shape),
                                    d_dx=numpy.zeros(shape),
                                    d_dy=numpy.zeros(shape),
                                    d2_dxdy=numpy.zeros(shape)),
                boundaries=grid
    ...
    def test_pi(self):
        ...
        for grid, test_points in zip(self._grids, self._test_points):
            shape = (grid['y'].size, grid['x'].size)
            psf = PiecewiseBicubicPSF(
                psf_parameters=dict(values=numpy.full(shape, numpy.pi),
                                    d_dx=numpy.zeros(shape),
                                    d_dy=numpy.zeros(shape),
                                    d2_dxdy=numpy.zeros(shape)),
                boundaries=grid
        ...
        def test_evaluation(psf, slope, grid, test_points):
            ...
            for point in test_points:
                ...
                self.assertEqual(psf(**point),
                                 ...
                                 '%f * %f + %f * %f'
                                 %
                                 (slope['x'], point['x'], slope['y'], point['y']))
            ...
            for x_slope in slopes:
                for y_slope in slopes:
                    grid_x, grid_y = numpy.meshgrid(grid['x'],
                                                    grid['y'])
                    psf = PiecewiseBicubicPSF(
                        psf_parameters=dict(values=grid_x * x_slope + grid_y * y_slope,
                                            d_dx=numpy.full(shape, x_slope),
                                            d_dy=numpy.full(shape, y_slope),
                                            d2_dxdy=numpy.zeros(shape)),
                        boundaries=grid
                    )
                    psf_slope = dict(x=x_slope, y=y_slope)
