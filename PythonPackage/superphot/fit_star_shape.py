...
class FitStarShape:
    ...
    Attributes:
        subpixmap:    The sub-pixel map, for PSF fitting only: a 2-D numpy
            array.
        ...
        shape_terms:    The terms the PSF is allowed to depend on. The EBNF
            ...
            list = "{" , term , { "," , term } , "}" ; (* simple listing of   *)
                                                       (* terms to include    *)
            
            poly = "O" , <integer> , list ; (* Expands to all polynomial      *)
                                            (* terms of up to combined order  *)
                                            (* <integer> of the entries in    *)
                                            (* list                           *)
            
            set = list | poly ;
            
            cross = set , { "*" , set } ; (* expands to the cross product of  *)
                                          (* all sets.                        *)
