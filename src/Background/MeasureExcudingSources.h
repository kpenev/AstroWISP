...
namespace Background {
    ...
    private:
        ...
        std::list< Point<double> >::const_iterator __source_iter;

        ///Excludes all pixels which have a mask other than GOOD.
        void exclude_mask(const FitsMask &mask);

    protected:
        ///The sources in the image
        std::list< Point<double> > _sources;
        
        ///The original image with the areas near sources masked.
        Image<double> _bg_values;
    public:
        ...
        virtual void add_source(const Point<double> &location)
        ...
        virtual Source operator()(const Point<double> &location) const
    ...
    MeasureExcludingSources::MeasureExcludingSources(
            ...
            const std::list< POINT_TYPE > &sources) :
        __aperture(exclude_aperture), _bg_values(image.values())
    {
        exclude_mask(image.mask());
        for(
