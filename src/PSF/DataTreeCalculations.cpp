...
namespace PSF {
    ...
    {
        if(data.get_optional<boost::any>("psffit.flux")) return;
        double mag_1adu=data.get<double>("psffit.magnitude_1adu",
                                         Core::NaN,
                                         IO::translate_double);
        IO::OutputArray<double>
            mag_array(data.get<boost::any>("psffit.mag"));
    ...
    void fill_psf_amplitudes(IO::H5IODataTree &data)
    {
        ...
        if(data.get_optional<boost::any>("psffit.amplitude")) return;
        fill_psf_fluxes(data);
        std::string psf_model=data.get<std::string>("psffit.model",
                                                    "",
                                                    IO::translate_string);
        if(psf_model=="bicubic" || psf_model=="zero") {
            data.put("psffit.amplitude",
