...
int main() 
{
	...
	int count=10000;
	size_t i;
	double test_data_double[count];
	int test_data_int[count];
	FILE *outf=fopen("test_output.dat", "wb");
	for(i=0; i<count; i++) {
		if (rand()<RAND_MAX/2) test_data_double[i]=0.0/0.0; 
		else test_data_double[i]=min_val+(max_val-min_val)*(
			(double)(rand())/RAND_MAX);
		test_data_int[i]=rand()-RAND_MAX/2;
	}
	bin_output(outf, test_data_double, count, DOUBLE_ID, precision);
	bin_output(outf, test_data_int, count, INT_ID);
	fclose(outf);
	FILE *inf=fopen("test_output.dat", "rb");
	int type_id;
	double *read_data_double=(double *)(bin_input(inf, &type_id));
	if (type_id!=DOUBLE_ID) printf("Wrong data type %d should be %d", 
			type_id, DOUBLE_ID);
	int *read_data_int=(int *)(bin_input(inf, &type_id));
	if (type_id!=INT_ID) printf("Wrong data type %d should be %d", 
			type_id, INT_ID);
	for(i=0; i<count; i++) {
		diff_double=test_data_double[i]-read_data_double[i];
		diff_int=test_data_int[i]-read_data_int[i];
		printf("%d %10.3e %10.3e %12.8f %12.8f %10d %13d %13d\n", (int)(i),
				0.5*precision, diff_double, test_data_double[i], 
				read_data_double[i], diff_int, test_data_int[i], 
				read_data_int[i]);
		if (fabs(diff_double)>0.5*precision || 
				((isnan(test_data_double[i]) || isinf(test_data_double[i])) ^
				 isnan(read_data_double[i]))) {
			printf("Written and read double data are not the same.\n");
			return 1;
		}
		if (read_data_int[i]!=test_data_int[i]) {
			printf("Written and read integer data are not the same.\n");
			return 1;
		}
	}
	free(read_data_double);
	free(read_data_int);
	return 0;
}*/
